...
void
print_src_port(const u_char *data, int offset)
{
...
void
print_dst_port(const u_char *data, int offset)
{
	int x_mask = 0x0000ff00;
...
void
print_seq_num(const u_char *data, int offset)
{
	int val =
	    ((data[offset+4] << 24 )
	    +(data[offset+5] << 16 )
...
void
print_ack_num(const u_char *data, int offset)
{
	int val =
	    ((data[offset+8] << 24 ) 
	    +(data[offset+9] << 16 )
...
void
print_data_off(const u_char *data, int offset)
{
	printf("\"tcp_head_len, %d\",", (data[offset+12] >> 4) * 4);
}

void
print_cntrl_bits(const u_char *data, int offset)
{
	printf("\"cntrl_bits, 0x%x\",", data[offset+13]);
}

void
print_window(const u_char *data, int offset)
{
	int x_mask = 0x0000ff00;
...
void
print_chksum_tcp(const u_char *data, int offset)
{
	int x_mask = 0x0000ff00;
