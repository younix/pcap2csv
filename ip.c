...
print_ip_fields(const u_char *data, int offset)
{
	...
	printf("\"version, 0x%02x\",", packet->version);
	printf("\"ihl, %d\",", packet->ihl * 4);
	printf("\"tos, 0x%02x\",", ntohs(packet->service));
	printf("\"datagram_len, %d\",", ntohs(packet->length));
	printf("\"id, 0x%04x\",", ntohs(packet->identity));
	...
	printf("\"header checksum, 0x%04x\",", ntohs(packet->chksum));
