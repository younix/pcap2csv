...
void
print_version_ip(const u_char *data, int offset)
{
	int mask = 0x0f;
...
void
print_ihl(const u_char *data, int offset)
{
	int mask = 0x0f;
...
void
print_tos_ip(const u_char *data, int offset)
{
	printf("\"tos, 0x%02x\",", data[offset+1]);
}

void
print_tot_len(const u_char *data, int offset)
{
...
void
print_id_ip(const u_char *data, int offset)
{
	printf("\"id, 0x%02x%02x\",", data[offset+4], data[offset+5]);
}

void
print_flags_ip(const u_char *data, int offset)
{
	int mask = 0x07;
...
void
print_frag_off(const u_char *data, int offset)
{
	int mask = 0x1F;

	printf("\"frag_off, 0x%02x%02x\",", (mask & data[offset+6]),
	    data[offset+7]);
}

void
print_ttl_ip(const u_char *data, int offset)
{
	printf("\"ttl, 0x%02x\",", data[offset+8]);
}

void
print_protocol(const u_char *data, int offset)
{
	printf("\"protocol, 0x%02x\",", data[offset+9]);
}

void
print_chksum(const u_char *data, int offset)
{
	printf("\"header checksum, 0x%02x%02x\",", data[offset+10],
	    data[offset+11]);
}

void
print_src_ip_addr(const u_char *data, int offset)
{
...
void
print_dst_ip_addr(const u_char *data, int offset)
{
